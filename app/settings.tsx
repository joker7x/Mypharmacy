...
import { printBarcodeLabel, isBluetoothPrintingAvailable, pairAndConnectPrinter, scanBluetoothPrinters } from "@/lib/printer-service";
...
export default function SettingsScreen() {
  ...
  const moveElement = (element: LabelElement, direction: -1 | 1) => {
    const index = printer.labelElements.indexOf(element);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= printer.labelElements.length) return;
    const next = [...printer.labelElements];
    [next[index], next[target]] = [next[target], next[index]];
    updatePrinter({ labelElements: next });
  };
  const printTest = async () => {
    if (!printer.savedPrinter) return Alert.alert("اربط الطابعة أولًا", "اختر طابعة Bluetooth واحفظها قبل طباعة ملصق تجريبي.");
    try {
      await printBarcodeLabel(printer.savedPrinter, printer, { name: "Panadol 500mg", barcode: "6221234567890", price: 72, expiryMonthYear: "8/2029" });
      Alert.alert("تم إرسال الملصق", "تحقق من مقاسه واتجاهه على الطابعة قبل الاستخدام اليومي.");
    } catch (error) { Alert.alert("تعذرت الطباعة", errorMessage(error)); }
  };

  return (
    <AdminShell title="إعدادات الصيدلية" subtitle="البيانات المحلية والطابعة الحرارية والملصقات">
      <Text style={adminStyles.sectionHeading}>الطابعة الحرارية</Text>
      <AdminCard>
        <View style={styles.printerStatus}>
          <View style={[styles.statusDot, printer.savedPrinter ? styles.statusConnected : styles.statusIdle]} />
          <View style={styles.printerStatusText}>
            <Text style={styles.printerName}>{printer.savedPrinter?.name ?? "لا توجد طابعة محفوظة"}</Text>
            <Text style={styles.printerAddress}>{printer.savedPrinter ? printer.savedPrinter.address : "ابحث عن Xprinter XP-233B ثم اربطها مرة واحدة"}</Text>
          </View>
        </View>
        ...
        {printers.map((candidate) => (
          <TouchableOpacity key={candidate.address} onPress={() => connect(candidate)} style={styles.printerRow} activeOpacity={0.76}>
            <View style={styles.printerRowText}><Text style={styles.deviceName}>{candidate.name}</Text><Text style={styles.deviceAddress}>{candidate.address}</Text></View>
            <Text style={styles.connectText}>{connectingAddress === candidate.address ? "جارٍ الربط…" : printer.savedPrinter?.address === candidate.address ? "مربوطة" : "ربط"}</Text>
          </TouchableOpacity>
        ))}
      </AdminCard>
...
const styles = StyleSheet.create({
  printerStatus: { flexDirection: "row-reverse", alignItems: "center", gap: 10, marginBottom: 14 },
  statusDot: { width: 11, height: 11, borderRadius: 6 }, statusConnected: { backgroundColor: COLORS.success }, statusIdle: { backgroundColor: COLORS.muted },
  printerStatusText: { flex: 1, alignItems: "flex-end" }, printerName: { color: COLORS.ink, fontSize: 14, fontWeight: "900", textAlign: "right" }, printerAddress: { color: COLORS.muted, fontSize: 10, marginTop: 3, textAlign: "right" },
  nativeNote: { color: COLORS.warning, backgroundColor: "#FFF8E9", fontSize: 10, fontWeight: "700", lineHeight: 16, textAlign: "right", padding: 10, borderRadius: 10, marginBottom: 10 },
  printerRow: { minHeight: 57, paddingVertical: 9, flexDirection: "row-reverse", alignItems: "center", gap: 10, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: COLORS.border }, printerRowText: { flex: 1, alignItems: "flex-end" }, deviceName: { color: COLORS.ink, fontSize: 12, fontWeight: "900" }, deviceAddress: { color: COLORS.muted, fontSize: 10, marginTop: 3 }, connectText: { color: COLORS.primary, fontSize: 11, fontWeight: "900" },
  presetRow: { flexDirection: "row-reverse", gap: 7 }, preset: { flex: 1, minHeight: 40, borderRadius: 13, backgroundColor: "#F3F2EF", alignItems: "center", justifyContent: "center", paddingHorizontal: 5 }, presetActive: { backgroundColor: COLORS.primary }, presetText: { color: COLORS.muted, fontSize: 10, fontWeight: "900" }, presetTextActive: { color: "#FFFFFF" },
  sizeFields: { flexDirection: "row-reverse", gap: 9, marginTop: 12 }, sizeField: { flex: 1 },
  elementRow: { minHeight: 54, flexDirection: "row-reverse", alignItems: "center", gap: 10 }, elementBorder: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: COLORS.border }, elementLabel: { flex: 1, color: COLORS.ink, fontSize: 12, fontWeight: "900", textAlign: "right" }, elementDisabled: { color: COLORS.muted }, reorder: { flexDirection: "row-reverse", gap: 3 }, orderButton: { width: 24, height: 28, alignItems: "center", justifyContent: "center" }, orderButtonText: { color: COLORS.primary, fontSize: 16, fontWeight: "900" }, orderDisabled: { color: "#CFD6D2" },
  previewCard: { alignItems: "center", backgroundColor: "#F7F6F2" }, labelPreview: { width: "82%", maxHeight: 210, minHeight: 110, backgroundColor: "#FFFFFF", borderWidth: 1.5, borderStyle: "dashed", borderColor: "#C9D2CD", borderRadius: 13, alignItems: "center", justifyContent: "center", padding: 12, gap: 5 }, previewName: { color: COLORS.ink, fontSize: 13, fontWeight: "900", textAlign: "center" }, barcode: { alignItems: "center", gap: 3, width: "86%" }, barcodeBars: { height: 38, width: "100%", flexDirection: "row", alignItems: "stretch", justifyContent: "space-between", transform: [{ scaleX: 0.92 }] }, barcodeBar: { height: "100%", backgroundColor: COLORS.deep }, previewBarcodeText: { color: COLORS.ink, fontSize: 9, writingDirection: "ltr" }, previewPrice: { color: COLORS.primary, fontSize: 14, fontWeight: "900" }, previewExpiry: { color: COLORS.muted, fontSize: 10, fontWeight: "800" }, previewCaption: { color: COLORS.muted, fontSize: 9, lineHeight: 15, textAlign: "center", marginTop: 12, marginBottom: 7 },
  bodyText: { color: COLORS.muted, fontSize: 11, lineHeight: 18, textAlign: "right", marginBottom: 12 }, retentionRow: { flexDirection: "row-reverse", gap: 8, alignItems: "center" }, retentionField: { flex: 1 }, retentionMark: { color: COLORS.primary, fontSize: 24, fontWeight: "900", marginTop: 17 },
});
