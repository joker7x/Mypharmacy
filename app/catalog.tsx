...
const formatPrice = (value: string | number) => `${new Intl.NumberFormat("ar-EG", { maximumFractionDigits: 2 }).format(Number(value))} ج.م`;
const formatUpdate = (value: number) => new Intl.DateTimeFormat("ar-EG", { day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }).format(new Date(value));
...
export default function CatalogScreen() {
  ...
  const statusQuery = trpc.catalog.status.useQuery(undefined, { refetchOnMount: "always" });
  const latestQuery = trpc.catalog.latest.useQuery({ limit: 20 });
  const searchQuery = trpc.catalog.search.useQuery({ query: canSearch ? query : "xx", limit: 30 }, { enabled: canSearch });
