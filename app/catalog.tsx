...
export default function CatalogScreen() {
  ...
  const latestQuery = trpc.catalog.latest.useQuery({ limit: PAGE_SIZE, offset, sort }, { enabled: !canSearch, staleTime: 60_000, refetchOnMount: false });
...
function Pagination({ page, pages, onPrevious, onNext }: { page: number; pages: number; onPrevious: () => void; onNext: () => void }) {
  ...
  return <View style={styles.pagination}><TouchableOpacity onPress={onNext} disabled={page >= pages - 1} style={[styles.pageButton, page >= pages - 1 && styles.pageDisabled]} activeOpacity={0.75}><Text style={styles.pageButtonText}>التالي</Text><IconSymbol name="chevron.left" size={18} color={COLORS.primary} /></TouchableOpacity><Text style={styles.pageLabel}>صفحة {page + 1} من {pages}</Text><TouchableOpacity onPress={onPrevious} disabled={page === 0} style={[styles.pageButton, page === 0 && styles.pageDisabled]} activeOpacity={0.75}><IconSymbol name="chevron.right" size={18} color={COLORS.primary} /><Text style={styles.pageButtonText}>السابق</Text></TouchableOpacity></View>;
}
