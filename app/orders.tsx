...
export default function OrdersScreen() {
  ...
  const handlePreview = useCallback((uri?: string) => setPreview(uri), []);
  const renderOrder = useCallback(({ item }: { item: IncomingOrder }) => <OrderCard order={item} onPreview={handlePreview} />, [handlePreview]);
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-background" className="flex-1"><FlatList data={incomingOrders} keyExtractor={(item) => item.id} renderItem={renderOrder} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" initialNumToRender={6} maxToRenderPerBatch={6} updateCellsBatchingPeriod={24} windowSize={5} contentContainerStyle={commonStyles.content} ItemSeparatorComponent={() => <View style={styles.listSeparator} />} ListHeaderComponent={<><TouchableOpacity onPress={() => router.back()} style={styles.back} activeOpacity={0.75}><IconSymbol name="chevron.right" size={20} color={COLORS.ink} /><Text style={styles.backText}>رجوع</Text></TouchableOpacity><PageHeader title="الطلبيات الواردة" subtitle="احتفظ بطلبات الموردين وصور الفواتير للرجوع إليها" action="إضافة طلب" onActionPress={() => setIsFormVisible(true)} /><Card style={styles.helperCard}><RoundIcon name="doc.text.fill" /><View style={styles.helperText}><Text style={styles.helperTitle}>كل طلب في مكانه</Text><Text style={styles.helperSubtitle}>أضف اسم الجهة، تفاصيل الطلب، وصورة الفاتورة من الكاميرا أو ألبوم الصور.</Text></View></Card><SectionTitle title={`الطلبات المحفوظة (${incomingOrders.length})`} /></>} ListEmptyComponent={<Card style={styles.emptyCard}><RoundIcon name="doc.text.fill" color={COLORS.muted} background="#EEF3F1" /><Text style={styles.emptyTitle}>لا توجد طلبيات محفوظة بعد</Text><Text style={styles.emptySubtitle}>عند وصول طلب من شركة أو مكتب، أضفه هنا وارفق صورة فاتورته.</Text><TouchableOpacity onPress={() => setIsFormVisible(true)} style={commonStyles.primaryButton} activeOpacity={0.85}><IconSymbol name="plus.circle.fill" size={19} color="#FFFFFF" /><Text style={commonStyles.primaryButtonText}>تسجيل أول طلب</Text></TouchableOpacity></Card>} />
